...
public class AssertTestCase extends TestCase {
	...
	public static void assertEquals(final String message,
		...
		assertEquals(message + " Arrays were not the same length.",
	...
	public static void assertEqualListContents(final List<?> expected,
			final List<?> actual) {
		assertEqualsCollectionContents(expected, actual);
	}

	public static void assertEqualsCollectionContents(
			final Collection<?> expectedCollection,
		...
		assertEquals("List was incorrect size.", expectedCollection.size(),
				actualCollection.size());
		final Iterator<?> actualCollectionIterator = actualCollection
				.iterator();
		int index = 0;
		for (final Object object : expectedCollection) {
			assertEquals("failed at index " + index, object,
					actualCollectionIterator.next());
			index++;
		}
	}
	...
	public void assertEquals(final int[] expected, final int[] actual) {
		...
		assertEquals("Arrays were not the same length.", expected.length,
				actual.length);
		...
		try {
			...
			for (int i = 0; i < expected.length; i++) {
				assertEquals("failed at position " + i
						+ " of the byte buffer: ", expected[i], actual[i]);
			}
		}

	}

	public void assertEquals(final String errorMessage, final byte[] expected,
			final byte[] actual) {
		try {
	...
	public static void assertEqualsCollectionContentsUnordered(
		...
		for (final Object object : expected) {
			assertTrue("Failed, did not contain " + object,
					actual.contains(object));
		}
	}

	public static <K, V> void assertEqualMapContents(
			final Map<K, V> expectedMap, final Map<K, V> actualMap) {
		assertNotNull("Expected map was null?", expectedMap);
		assertNotNull("Actual map was null", actualMap);
		assertEquals("Map has incorrect size", expectedMap.size(),
				actualMap.size());
	...
	public static <T> T assertIsOfTypeAndGet(final Class<T> expectedType,
			final Object actual) {
	...
	public static <T> T assertIsOfTypeAndGet(final String errorMessage,
		...
		assertNotNull(errorMessage + " Object was null instead of type "
				+ expectedType.getSimpleName(), actual);
		assertEquals(errorMessage + " Object " + actual + " was not of type "
				+ expectedType.getSimpleName(), expectedType, actual.getClass());
		return (T) actual;
	}

	public static void assertEqualCollectionContentsRegardlessOfOrder(
			final Collection<?> expected, final Collection<?> actual) {
		assertEquals(expected.size(), actual.size());
		for (final Object object : expected) {
			assertTrue(
					"Object in 1st list not found in 2nd. " + object.toString(),
					actual.contains(object));
		}
	}

	public void assertEqualSetContents(final Set<?> expectedSet,
			final Set<?> actualSet) {
	...
	public static void assertClassImplementsInterfaces(
			final Class<?> classUnderTest, final Class<?>... expectedInterfaces) {
		final Class<?>[] interfaces = classUnderTest.getInterfaces();
		assertEquals("Wrong Number of Interfaces.", expectedInterfaces.length,
				interfaces.length);
		assertEquals("Wrong interfaces.", expectedInterfaces, interfaces);
	}

	public static void assertImplementsInterface(
			final Class<?> expectedInterface, final Class<?> theClass) {

		assertEquals(theClass.getSimpleName()
				+ " does not implement exactly 1 interface. ", 1,
	...
	public static void assertImplementsInterfaces(final Class<?> theClass,
		...
		for (final Class<?> expectedInterface : expectedInterfaces) {
			...
			assertTrue(
					"Missing " + expectedInterface.getSimpleName(),
					Arrays.asList(theClass.getInterfaces()).contains(
							expectedInterface));
		}
	}
	...
	public static <T> void assertHasOneInstanceOfThisType(
			final Class<T> classy, final Object[] array) {
		int numberFound = 0;
		String classesFound = "";
		...
		assertEquals("Did not find an instance of " + classy.getName()
				+ " but did find < " + classesFound, 1, numberFound);
	}

	public static <T> int getFirstIndexOfThisType(final Class<T> classy,
			final Object[] array) {
		int index = 0;
		boolean found = false;
		for (final Object object : array) {
	...
	public void assertConstructorIsPackageProtected(final Class<?> class1) {
		final Constructor<?>[] declaredConstructors = class1
		...
		try {
			...
			final String msg = class1.getSimpleName()
					+ " should have a single protected constructor.";
			assertTrue(msg, Modifier.isProtected(declaredConstructors[0]
					.getModifiers()));
		}
	}
	...
	public void assertConstructorIsPrivate(final Class<?> class1) {
		...
		assertEquals(1, declaredConstructors.length);
		final String msg = class1.getSimpleName()
				+ " should have a single private constructor.";
		assertTrue(msg,
